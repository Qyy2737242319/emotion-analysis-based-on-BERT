...
flags.DEFINE_string("task_name", "classify", "The name of the task to train.")#调整模式参数

flags.DEFINE_string("vocab_file", "./tudouNLP/models/vocab.txt",
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "output_dir", "./tudouNLP/models/output",
...
class classify_text(DataProcessor):
    ...
    def get_labels(self):
        """See base class."""
        #return FLAGS.label_list#label_list为标签列表，此处为情感分析故只需返回0或1
        return ["0","1","2"]
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    if FLAGS.task_name.lower() == 'tag' or FLAGS.task_name.lower() == 'ner':
        ...
        hidden_size = output_layer.shape[-1]#返回值为整数，output_layer.shape[-1].value改为output_layer.shape[-1]
        ...
        with tf.compat.v1.variable_scope("loss"):
            if is_training:
                # I.e., 0.1 dropout
                #output_layer = tf.nn.dropout(output_layer, keep_prob=0.9)#Got an unexpected keyword argument 'keep_prob',use below instead
...
def main(_):
    ...
    processors = {
        "pair": sentence_pair,#句子配对
        "classify": classify_text,#情感分析（分类）
        "ner": NER_processor,#实体识别
        "tag": tagging_processor#序列标注
    }#识别模式参数
