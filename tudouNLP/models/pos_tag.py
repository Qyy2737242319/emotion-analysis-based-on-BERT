...
flags.DEFINE_string("vocab_file", './tudouNLP/modelsvocab.txt',
...
flags.DEFINE_string(
    "init_checkpoint", './tudouNLP/models/model/pre_trained_model/checkpoint',
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    hidden_size = output_layer.shape[-1]#int类型故将output_layer.shape[-1].value改为output_layer.shape[-1]
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        if task_name == 'tag':
            #print(os.getcwd())
            with open(os.path.join('./tudouNLP/models/', 'pos_tag.pkl'), 'rb') as rf:
