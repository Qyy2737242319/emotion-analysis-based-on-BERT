...
class BertConfig(object):
  ...
  def from_json_file(cls, json_file):
    #print(os.getcwd())
    os.chdir("F:\Program Files (x86)//tudouNLP")
    """Constructs a `BertConfig` from a json file of parameters."""
    with tf.compat.v1.gfile.GFile(json_file) as reader:#r改为rb，二进制模式
      text = reader.read()
      #print(text)
    return cls.from_dict(json.loads(text))#更改为汉字编码格式gbk/gb2312/gb18030
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
  ...
  从checkpoint的list_variables中，获取tvars（一般是tvars = tf.compat.v1.trainable_variables()）中的变量
  """
